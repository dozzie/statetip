...
reload() ->
  {ok, NewAddrs} = application:get_env(statip, senders),
...
stop_child(Name, Pid) ->
  statip_sender_listen:shutdown(Pid),
...
init([] = _Args) ->
  {ok, Addrs} = application:get_env(senders),
...
listen_child(Address, Port) ->
  {child_name(Address, Port),
    ...
    transient, 5000, worker, [statip_sender_listen]}.
